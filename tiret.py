...
def fetch_repo_info(owner, repo, token):
	...
	API. It requires a GitHub personal access token (PAT) to authenticate the
	requests.
	...
	Raises:
		RuntimeError: if a request to the GitHub API fails. Status code 401
			can be due to an incorrect PAT.
	"""
	headers = {_KEY_AUTHORIZATION: token}
	...
	contributors = _fetch_repo_contributors(repo_url, headers)
	commits = _fetch_repo_num_commits(repo_url, headers)
	languages = _fetch_repo_languages(repo_url, headers)
	repo = Repository(
...
def _fetch_repo_contributors(repo_url, headers):
...
def _fetch_repo_languages(repo_url, headers):
	repo_lang_url = repo_url + _SLASH + KEY_LANG
	lang_response = requests.get(repo_lang_url, headers=headers)
...
def _fetch_repo_num_commits(repo_url, headers):
	...
	while True:
		commit_response = requests.get(
			repo_commit_url+str(page_num), headers=headers)
